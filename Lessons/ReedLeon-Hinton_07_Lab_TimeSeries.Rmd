...
BRA_Rainfall_ts <- ts(data = BRA_complete$Rainfall, start = c(1901, 1), frequency = 12)
BRA_Temperature_ts <- ts(data = BRA_complete$Temperature_C, start = c(1901, 1), frequency = 12)
...
BRA_Rainfall_Components <- as.data.frame(BRA_Rainfall_decomp$time.series[,1:3])
BRA_Temperature_Components <- as.data.frame(BRA_Temperature_decomp$time.series[,1:3])
...
ggplot(BRA_Rainfall_Components) +
  ...
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff", size = 0.5)
