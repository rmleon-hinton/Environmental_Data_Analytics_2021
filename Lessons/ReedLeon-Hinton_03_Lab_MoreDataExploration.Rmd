...
vector1[3] # looking at the third element in the vector1 vector of numbers. 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # Makes a matrix with values from 1 to 20 which places these values in a 5 by 4 matrix
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #changing it to run the operation by row instead of by column
matrix2
...
matrix1[4, ] # looks at the 4th row of the matrix
matrix1[ , 3] # looks at the third column of the matrix
matrix1[c(12, 14)] #looks at the 12th and 14th component in the matrix
matrix1[c(12:14)] #looks at the 12th through the 14th components in the matrix
matrix1[2:4, 1:3] #shows rows 2 to 4 and columns 1 to 3

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # Naming the rows and columns
matrix4
...
dataframe1 <- data.frame(d,e,f) # making a data frame with the three previous vectors in it
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # naming the columns in the data frame

dataframe1[1:2,] # Getting the first 2 rows of the data frame
dataframe1[c("ID","Passed")] # only looking at those two columns
dataframe1$ID
...
```{r}
# creating the high temperature vector
hightemp <- c(49, 52, 50, 52, 52, 52, 51, 41, 42, 36)

# creating the low temperature vector
lowtemp <- c(25, 40, 28, 33, 28, 35, 33, 26, 22, 20)
...
```{r}
# creating a function to convert the temperature
tempconversion <- function(t){
  (t-32)*(5/9)
}

# converting the high temperature vector to c
hightempc <- tempconversion(hightemp)

# converting the low temperature vector to c
lowtempc <- tempconversion(lowtemp)
...
```{r}
# creating the data frame with the different temperatures inside
tempframe <- data.frame(hightemp, lowtemp, hightempc, lowtempc)

#naming the data frame columns
names(tempframe) <- c("High_Temperature_(F)", "Low_Temperature_(F)", "High_Temperature_(C)", "Low_Temperature_(C)")
```
...
#running the sd function to get standard deviation of each column
std_hightemp <- sd(tempframe$`High_Temperature_(F)`)
std_hightemp
std_lowtemp <- sd(tempframe$`Low_Temperature_(F)`)
std_lowtemp
std_hightempc <- sd(tempframe$`High_Temperature_(C)`)
std_hightempc
std_lowtempc <- sd(tempframe$`Low_Temperature_(C)`)
std_lowtempc
```
