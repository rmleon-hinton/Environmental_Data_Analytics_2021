...
> Answer: The console runs an operation showing you the different aspects of the package which were loaded. For example, with tidyverse, it shows all other packages loaded alongside. This is if you are installing a package. If you are simply adding it to the library, nothing shows up in the console aside from simply library(tidyverse). 
...
> ANSWER: The README file is located in the "Metadata" folder within the "Data" folder that is in the main folder for this local version of the GitHub repository. It shows what this data was prepared for along with where it was sourced from. Then, it explains how to reproduce the conditions under which it was collected, to allow someone else to follow along even without the original dataset. Using these methods, it makes reproducing the analysis completed much easier. 
...
> Answer: They became NA values, which then made up the majority of the data entries within the file. 
...
> Answer: In time series forecasting and analysis you utilize other methods to replace the NAs from the dataset if they are in the middle, but you can also remove them if they are at the beginning of your time period. If you were conducting a survey and had voluntary, participant data you would want to remove NA values, as you are not recording values in any particular order. 
...
```{r fixformartDate}
# formatting the current datetime column back into a character format
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# creating an ifelse statement to determine which dates are actually in the 1900s rather than in the 2000s
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })
# using the above function to create the earlier dates
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#Changing the format back to the correct date format with the correct years
