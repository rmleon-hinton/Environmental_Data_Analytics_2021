...
class(NTL.phys.data$year4) # this is an integer
...
NTL.phys.data.Tuesday <- droplevels(NTL.phys.data.Tuesday)

```
Question: Why don't we filter using row numbers?

> Answer: It is not location based, meaning that if you add data or re-order the data this code will still work. This makes the function more adaptable.
...
NTL.phys.data.summary <-
  ...
  mutate(temperature_F = (temperature_C*9/5)+32) %>%
  summarise(meantemp_F = mean(temperature_F),
            meantemp_C = mean(temperature_C))
...
NTL.phys.data.spread <- pivot_wider(NTL.phys.data, names_from = "depth", values_to = "temperature_C")
