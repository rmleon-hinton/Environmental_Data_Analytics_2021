...
ggplot(PeterPaul.surface, aes(x = daynum, y = temperature_C, colour = year4)) + 
  geom_point() +
  ylab("Temperature (C)") +
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, colour = depth)) + 
  ...
  ylab("Temperature (C)") +
  xlab("Date") +
  scale_color_viridis(option = "magma", direction = -1) # -1 makes more sense because it gets darker as it gets deeper. 
...
ggplot(EPAair, aes(x = PM2.5, y = Ozone, colour = SITE_LATITUDE)) + 
